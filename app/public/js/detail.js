...
var template = __webpack_require__(468), timeline = __webpack_require__(470), uploadpanel = __webpack_require__(471), appInfo = __webpack_require__(469), willOnLine = __webpack_require__(472), footerTpl = __webpack_require__(47);
var uploadData;
var id;
var uid;
var appinfo;
...
var detail = (function (_super) {
    ...
    detail.prototype.beforeRender = function () {
        var _this = this;
        //获取url中参数
        id = url_1.queryToObj().id;
        return request_1.get("./appdist/app/" + id, {}).then(function (_a) {
            var data = _a.data;
            appinfo = data.beans;
            ...
            _this.errorBar("连接超时，请重新登录，正在跳回登录页...", "overTime");
        });
    };
    detail.prototype.afterRender = function () {
        var _this = this;
        var hash = (window.location.hash.length > 0 ? window.location.hash.substring(1) : ""), pageApp = this.domNode.querySelector('#page-app'), appScope = this.domNode.querySelector('#app-scope');
        pageApp.className = "";
        dom_1.addClass(pageApp, 'page-app', "app-" + hash);
        if (hash == '') {
            request_1.get("./appdist/applist/" + bundleid, {}).then(function (_a) {
                var data = _a.data;
                dom_1.empty(appScope);
                _this.renderTplToNode(timeline, data.beans, appScope, dom_1.IPosition.LastIn);
            });
        }
        else {
            if (hash == 'info') {
                ...
                dom_1.empty(appScope);
                this.renderTplToNode(appInfo, appinfo, appScope, dom_1.IPosition.LastIn);
            }
            else {
                dom_1.empty(appScope);
                this.renderTplToNode(willOnLine, appinfo, appScope, dom_1.IPosition.LastIn);
            }
        }
        this.renderTplToNode(footerTpl, {}, this.domNode.querySelector(".footer-wrapper"), dom_1.IPosition.LastIn);
    };
    // hash改变的时候重新render
    detail.prototype.onHashChange = function (evt) {
        evt.preventDefault ? evt.preventDefault() : evt.returnValue = false;
        var evtTarget = evt.target;
        dom_1.removeClass(this.domNode.querySelector('#toggle-active .active'), 'active');
        dom_1.addClass(evtTarget, 'active');
        window.location.hash = evtTarget.getAttribute('data-hash');
        this.afterRender();
    };
    detail.prototype.onHashClear = function () {
        dom_1.removeClass(this.domNode.querySelector('#toggle-active .active'), 'active');
        window.location.hash = "";
        this.afterRender();
    };
    // 退出登录
    detail.prototype.onLogout = function () {
        request_1.post("./appdist/logout", {}).then(function () {
            window.location.href = 'login.html';
        });
    };
    detail.prototype.onUploadFile = function () {
        var _this = this;
        var uploadwarp = this.domNode.querySelector('#uploadwarp');
        $('#uploadApp').fileupload({
            dataType: 'json',
            url: config_1.get().BASE_URL + "appdist/upload",
            add: function () {
                var args = [];
                for (var _i = 0; _i < arguments.length; _i++) {
                    args[_i] = arguments[_i];
                }
                dom_1.removeClass(uploadwarp, 'app-hide');
                args[1].submit();
            },
            done: function () {
                var args = [];
                ...
                // 上传加载父容器的隐藏——添加
                dom_1.addClass(uploadwarp.querySelector('.state-parsing'), 'app-hide');
                // 根据获取的数据对新版本表单进行渲染
                _this.renderTplToNode(uploadpanel, uploadData, uploadwarp.querySelector('#stateForm'), dom_1.IPosition.LastIn);
            },
            fail: function () {
                dom_1.removeClass(uploadwarp.querySelector('.parse-error-content'), 'app-hide');
            }
        });
    };
    //提交上传版本的修改
    detail.prototype.onUploadEdit = function () {
        var _this = this;
        var stateForm = this.domNode.querySelector('#stateForm');
        var appDetail = {
            "uid": uid,
        ...
        request_1.post('./appdist/addappinfo', {
            data: appDetail,
            responseType: 'json',
            timeout: 1000,
            async: true
            ...
            _this.afterRender();
            _this.onCloseUpnload();
        }, function () {
            // 提交失败——提示错误
            dom_1.addClass(stateForm.querySelector('.action button'), 'app-hide');
            dom_1.removeClass(stateForm.querySelector('.action .upload-failed-tips'), 'app-hide');
        });
    };
    ;
    // 关闭上传
    detail.prototype.onCloseUpnload = function () {
        // 获取上传父容器
        var uploadwarp = this.domNode.querySelector('#uploadwarp');
        // 上传加载父容器的隐藏——去除
        dom_1.removeClass(uploadwarp.querySelector('.state-parsing'), 'app-hide');
        // 上传错误的隐藏——添加
        dom_1.addClass(uploadwarp.querySelector('.parse-error-content'), 'app-hide');
        // 上传父容器的隐藏——添加
        dom_1.addClass(uploadwarp, 'app-hide');
        // 新版本信息表单——清除
        dom_1.empty(uploadwarp.querySelector('#stateForm'));
    };
    //开始修改版本信息
    detail.prototype.onStartEdit = function (evt) {
        this.toggleHide(evt, '.app-edited', '.app-editing');
    };
    //结束修改版本信息
    detail.prototype.onEndEdit = function (evt) {
        var thisedit = this.findClosest(evt.target, 'directive-view-release');
        //如果返回值类型为string则判断为查找失败，输出错误并return掉。
        if ((typeof thisedit) === 'string') {
            console.log(thisedit);
            return;
        }
        thisedit.querySelector("textarea.app-editing").value("");
        this.toggleHide(evt, '.app-editing', '.app-edited');
    };
    //提交修改版本信息
    detail.prototype.onSubEdit = function (evt) {
        var _this = this;
        var thisedit = this.findClosest(evt.target, 'directive-view-release');
        //如果返回值类型为string则判断为查找失败，输出错误并return掉。
        if ((typeof thisedit) == 'string') {
        ...
        var queryVal = thisedit.querySelector("textarea.app-editing").value;
        if (queryVal !== '') {
            var getId = thisedit.querySelector(".icon-upload-cloud2").getAttribute('data-appId');
            request_1.post("./appdist/appinfoedit/" + getId, { data: { description: queryVal } }).then(function (_a) {
                var data = _a.data;
                var retdata = data.beans;
                var desc = retdata.description;
                ...
                var data = _a.data;
                if (data == '用户不存在') {
                    _this.errorBar("用户不存在，请重新登录，正在跳回登录页...", "overTime");
                }
                else if (data == '修改失败') {
                    _this.errorBar("修改失败，请重新操作...");
                }
                else {
                    _this.errorBar("未知错误发生，请重新操作...");
                }
            });
            thisedit.querySelector("textarea.app-editing").value("");
            this.toggleHide(evt, '.app-editing', '.app-edited');
    ...
    //基本信息
    detail.prototype.onEdit = function () {
        var namevalue = this.domNode.querySelector('.app-name .appname').value;
        var desValue = this.domNode.querySelector('.form-control').value;
        namevalue = this.trim(namevalue);
        desValue = this.trim(desValue);
        if (appname !== namevalue || description !== desValue) {
            this.domNode.querySelector('.apps-app-info .save').disabled = false;
            dom_1.removeClass(this.domNode.querySelector('.apps-app-info .cancel'), 'app-hide');
        }
    };
    detail.prototype.onCancel = function () {
        this.domNode.querySelector('.app-name .appname').value = appname;
        this.domNode.querySelector('.form-control').value = description;
        this.domNode.querySelector('.apps-app-info .save').disabled = true;
        dom_1.addClass(this.domNode.querySelector('.apps-app-info .cancel'), 'app-hide');
    };
    detail.prototype.onEditSave = function () {
        var _this = this;
        var namevalue = this.domNode.querySelector('.app-name .appname').value;
        var desValue = this.domNode.querySelector('.form-control').value;
        request_1.post("./appdist/appinfoedit/" + id, {
            ...
            var data = _a.data;
            appinfo = data.beans;
            _this.domNode.querySelector('.secondary-title').innerHTML = namevalue;
            _this.onHashClear();
        }, function () {
            dom_1.addClass(_this.domNode.querySelector('.apps-app-info .info-btns'), 'app-hide');
            dom_1.removeClass(_this.domNode.querySelector('.apps-app-info .sucMes'), 'app-hide');
            _this.domNode.querySelector('.apps-app-info .save').disabled = true;
            dom_1.addClass(_this.domNode.querySelector('.apps-app-info .cancel'), 'app-hide');
            setTimeout(function () {
                dom_1.addClass(_this.domNode.querySelector('.apps-app-info .sucMes'), 'app-hide');
                dom_1.removeClass(_this.domNode.querySelector('.apps-app-info .info-btns'), 'app-hide');
            }, 2000);
        });
    };
    /**
     ...
     */
    detail.prototype.toggleHide = function (evt, addName, remName) {
        var thisedit = this.findClosest(evt.target, 'directive-view-release');
        //如果返回值类型为string则判断为查找失败，输出错误并return掉。
        if ((typeof thisedit) === 'string') {
            console.log(thisedit);
        ...
        var queryEle = thisedit.querySelectorAll(addName);
        dom_1.addClass(queryEle, 'app-hide');
        queryEle = thisedit.querySelectorAll(remName);
        dom_1.removeClass(queryEle, 'app-hide');
    };
    /**
     *
     *
     ...
     */
    detail.prototype.findClosest = function (findEle, findCls) {
        if (findEle.parentNode === null) {
            return "未找到含有指定class的元素";
        }
        findEle = findEle.parentNode;
        var isHas = dom_1.hasClass(findEle, findCls);
        if (isHas) {
            return findEle;
        }
        else {
            var ret = this.findClosest(findEle, findCls);
            return ret;
        }
    };
    /**
     ...
     */
    detail.prototype.trim = function (str) {
     ...
     */
    detail.prototype.errorBar = function (text, type) {
        var _this = this;
        this.domNode.querySelector('#errBar p').innerHTML = text;
        dom_1.removeClass(this.domNode.querySelector('#errBar'), 'app-hide');
        if (type && type == 'overTime') {
            ...
            setTimeout(function () {
                dom_1.addClass(_this.domNode.querySelector('#errBar'), 'app-hide');
                _this.domNode.querySelector('#errBar p').innerHTML = "";
            }, 2000);
        }
    };
    return detail;
...
module.exports = (Handlebars["default"] || Handlebars).template({"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
  ...
  return "<div class=\"page-tabcontent apps-app-info\">\n    <div class=\"middle-wrapper\" ng-show=\"activitiesReady\">\n        <form class=\"app-info-form\" name=\"appInfoForm\">\n            <div class=\"field app-id\">\n                <div class=\"left-label\">应用 ID</div>\n                <div class=\"value\"><input readonly=\"readonly\" value=\""
    + alias4(((helper = (helper = helpers.id || (depth0 != null ? depth0.id : depth0)) != null ? helper : alias2),(typeof helper === alias3 ? helper.call(alias1,{"name":"id","hash":{},"data":data}) : helper)))
    + "\"></div>\n            </div>\n            <hr>\n            <div class=\"field app-name\">\n                <div class=\"left-label\">应用名称</div>\n                <div class=\"value\"><input type=\"text\" class=\"appname\" value=\""
    + alias4(((helper = (helper = helpers.appname || (depth0 != null ? depth0.appname : depth0)) != null ? helper : alias2),(typeof helper === alias3 ? helper.call(alias1,{"name":"appname","hash":{},"data":data}) : helper)))
    + "\" data-event=\"keyup:edit\"></div>\n            </div>\n            <div class=\"field app-id\">\n                <div class=\"left-label\">应用描述</div>\n                <div class=\"value\"><textarea class=\"form-control\" data-event=\"keyup:edit\">"
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data) {
  ...
  return "            <li>\n                <div class=\"directive-view-release\">\n                    <i class=\"icon-upload-cloud2\" data-appId="
    + alias2(alias1((depth0 != null ? depth0.id : depth0), depth0))
    + "></i>\n                    <b>"
    + alias2(alias1((depth0 != null ? depth0.version : depth0), depth0))
    + " (Build "
    + alias2(alias1((depth0 != null ? depth0.build : depth0), depth0))
    + ")</b>\n                    <div class=\"release-metainfo\">\n                        <small>\n                            <i class=\"icon-calendar\"></i> \n                            <span>"
    + alias2(__default(__webpack_require__(514)).call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.updated_at : depth0),{"name":"./formatDate","hash":{},"data":data}))
    + "</span>\n                        </small>\n                    </div>\n                    <pre class=\"app-edited\">"
    + alias2(alias1((depth0 != null ? depth0.description : depth0), depth0))
    + "</pre>\n                    <textarea class=\"app-editing app-hide\" placeholder="
    + alias2(alias1((depth0 != null ? depth0.description : depth0), depth0))
    ...
    + " MB</span>\n                        </button>\n                        <a href=\"./downloadPage.html?id="
    + alias2(alias1((depth0 != null ? depth0.id : depth0), depth0))
    + "\" target=\"_blank\"><i class=\"icon-eye\"></i> <span>预览</span></a>\n                    </div>\n                </div>\n            </li>\n";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
    var stack1;

  return "<div class=\"page-app-activities page-tabcontent\">\n    <div class=\"middle-wrapper\">\n        <ul class=\"list-unstyled time-line\">\n"
...
module.exports = (Handlebars["default"] || Handlebars).template({"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
  ...
  return "\n<div class=\"header\">\n    <div class=\"pull-left appicon\"><img src=\""
    + alias4(((helper = (helper = helpers.iconurl || (depth0 != null ? depth0.iconurl : depth0)) != null ? helper : alias2),(typeof helper === alias3 ? helper.call(alias1,{"name":"iconurl","hash":{},"data":data}) : helper)))
    + "\"></div>\n    <div class=\"release-info\">\n        <span class=\"version\">"
    + alias4(((helper = (helper = helpers.version || (depth0 != null ? depth0.version : depth0)) != null ? helper : alias2),(typeof helper === alias3 ? helper.call(alias1,{"name":"version","hash":{},"data":data}) : helper)))
    + " (Build "
    + alias4(((helper = (helper = helpers.build || (depth0 != null ? depth0.build : depth0)) != null ? helper : alias2),(typeof helper === alias3 ? helper.call(alias1,{"name":"build","hash":{},"data":data}) : helper)))
    + ")</span>\n    </div>\n    <div class=\"appname\"><input type=\"text\" id=\"nameInput\" value=\""
    + alias4(((helper = (helper = helpers.appname || (depth0 != null ? depth0.appname : depth0)) != null ? helper : alias2),(typeof helper === alias3 ? helper.call(alias1,{"name":"appname","hash":{},"data":data}) : helper)))
...
function __default(obj) { return obj && (obj.__esModule ? obj["default"] : obj); }
module.exports = (Handlebars["default"] || Handlebars).template({"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
    return "<div class=\"page-tabcontent willOnLine\">\n    <h1>即将上线...</h1>\n</div>";
},"useData":true});
...
var detail_1 = __webpack_require__(266);
new detail_1["default"]('#detail');
...
var webDate = __webpack_require__(191);
